...
namespace DAL
{
    public class Activity
    {
        ...
        public int GetVerifyCount()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select count(1) from activity where activityVerify='审核成功'";
        ...
        public IEnumerable<Model.Activity> GetNew()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from activity where activityVerify='审核成功' and endTime>=now() order by endTime desc limit 2";
        ...
        public Model.Activity GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from activity where activityVerify='审核成功' and recommend='是' and endTime>=now order by recommendTime desc limit 1";
        ...
        public Model.Activity GetEnd()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from activity where activityVerify='审核成功' and endTime<now() order by endTime desc limit 1";
                return cn.QueryFirstOrDefault<Model.Activity>(sql);
            }
        }
        ...
        public int UpdateRecommend(Model.Activity activity)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update activity set recommend=@commend,recommendTime=@recommendTiem where activityid=@activityId";
